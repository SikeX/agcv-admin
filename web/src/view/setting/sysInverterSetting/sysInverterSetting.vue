  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column sortable align="left" label="逆变器编号" prop="inverterNo" width="120" />
   ...
   <template #default="scope">{{ formatDate(scope.row.updatedAt) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateSysInverterSettingFunc(scope.row)">编辑</el-button>
...
import {
  ...
  deleteSysInverterSetting,
  deleteSysInverterSettingByIds,
  updateSysInverterSetting,
  findSysInverterSetting,
  getSysInverterSettingList
...
const getTableData = async() => {
  const table = await getSysInverterSettingList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteSysInverterSettingFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSysInverterSettingByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSysInverterSettingFunc = async(row) => {
    const res = await findSysInverterSetting({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSysInverterSettingFunc = async (row) => {
    const res = await deleteSysInverterSetting({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
