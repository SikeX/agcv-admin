...
func (s *coapReceiver) handleDataReceive(w mux.ResponseWriter, r *mux.Message) {
    ...
    if err != nil {
        ...
        s.sendErrorResponse(w, r, message.BadRequest, "读取请求体失败")
        return
    }
    ...
    if err := json.Unmarshal(body, &dataMessages); err != nil {
        ...
        s.sendErrorResponse(w, r, message.BadRequest, "数据格式错误")
        return
    }
...
func (s *coapReceiver) sendSuccessResponse(w mux.ResponseWriter, r *mux.Message, msg string) {
    ...
    if err := w.SetResponse(message.Content, message.AppJSON, bytes.NewReader(data)); err != nil {
        global.GVA_LOG.Error("发送CoAP响应失败", zap.Error(err))
    }
}

// sendErrorResponse 发送错误响应
func (s *coapReceiver) sendErrorResponse(w mux.ResponseWriter, r *mux.Message, code message.Code, msg string) {
    response := map[string]interface{}{
        "code": -1,
        "msg":  msg,
    }
