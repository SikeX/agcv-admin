...
func serveCoap(conn *net.UDPConn) {
    ...
    for {
        ...
        // 查找对应的处理器
        handler := getCoapHandler(path, msg.code)
        if handler != nil {
            // 调用处理器
            ctx := context.Background()
            respCode, payload = handler(ctx, msg)
        } else {
            // 未找到处理器
            if _, pathExists := coapRoutes[path]; pathExists {
                // 路径存在但方法不支持
...
func buildCoapResponse(req coapMessage, respType int, code byte, payload []byte) []byte {
    ...
    header[0] = byte(coapVersion<<6) | byte((respType&0x03)<<4) | byte(tkl)
    header[1] = code
