...
// GetSysInverterSettingByInverterNo 根据逆变器编号获取逆变器设置记录
// Author [yourname](https://github.com/yourname)
func (sysInverterSettingService *SysInverterSettingService) GetSysInverterSettingByInverterNo(ctx context.Context, inverterNo string) (sysInverterSetting setting.SysInverterSetting, err error) {
	err = global.GVA_DB.Where("inverter_no = ?", inverterNo).First(&sysInverterSetting).Error
	return
}
