...
func (agvcQxyHisService *AgvcQxyHisService) GetAgvcQxyHistory(ctx context.Context, eqid, startTime, endTime string) ([]map[string]interface{}, error) {
    ...
    from(bucket: "%s")
        ...
        |> filter(fn: (r) => r["eqType"] == "8")
    `, global.GVA_CONFIG.InfluxDB.GetAgvcBucket(), startTime, endTime, global.GVA_CONFIG.InfluxDB.GetMeasurement(), eqid)
	...
	if len(historyData) == 0 {
		global.GVA_LOG.Info(fmt.Sprintf("气象仪%s在时间范围%s到%s内没有历史数据", eqid, startTime, endTime))
