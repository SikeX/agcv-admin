...
func (agvcAvcHisService *AgvcAvcHisService) GetAvcRealData(ctx context.Context, psid int, eqid int) (*agvc.AgvcAvc, error) {
	// 检查InfluxDB客户端是否已初始化
	if global.GVA_INFLUXDB == nil {
		return nil, fmt.Errorf("InfluxDB客户端未初始化")
	}
	...
	result, err := queryAPI.Query(ctx, flux)
	if err != nil {
		global.GVA_LOG.Error(fmt.Sprintf("查询AVC %s的实时数据失败: %v", eqid, err))
		return nil, err
	}

	// 创建结果对象
	realData := &agvc.AgvcAvc{
		Psid: &psid,
		Eqid: &eqid,
	}
	...
	for result.Next() {
		...
		// 根据point值设置对应的字段
		setFieldByPointForAvcRealData(realData, point, value)
	}

	// 检查是否有错误
	if result.Err() != nil {
		global.GVA_LOG.Error(fmt.Sprintf("解析AVC %s的实时数据失败: %v", eqid, result.Err()))
		return nil, result.Err()
	}

	// 如果没有数据
	if !hasData {
		return nil, fmt.Errorf("未找到AVC %s的实时数据", eqid)
	}
