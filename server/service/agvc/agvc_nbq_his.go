...
import (
    ...
    "github.com/flipped-aurora/gin-vue-admin/server/service/agvc/utils"
...
func (agvcNbqHisService *AgvcNbqHisService) GetNbqRealData(ctx context.Context, info agvcReq.AgvcNbqRealDataRequest) (interface{}, int64, error) {
    ...
    // 调用通用实时数据服务
    realService := &AgvcRealService{}

    res := make([]*agvc.AgvcNbq, 0)

    for _, setting := range inverterSettings {
        psid := setting.Psid
        inverterNo := setting.InverterNo
        // 调用通用实时数据服务
        realData, err := realService.GetRealData(ctx, *psid, *inverterNo, NBQ_DEVICE_TYPE)
        if err != nil {
            return nil, 0, err
        }
        if realData == nil {
        ...
        realData.(*agvc.AgvcNbq).Name = setting.Name
        res = append(res, realData.(*agvc.AgvcNbq))
    }
...
func (agvcNbqHisService *AgvcNbqHisService) WriteAgvcNbqDataToInfluxDB(ctx context.Context, psid int, eqid int, point string, value float64) error {
    ...
    // 获取写API
    writeAPI := global.GVA_INFLUXDB.WriteAPIBlocking(global.GVA_CONFIG.InfluxDB.Org, bucket)
...
func (agvcNbqHisService *AgvcNbqHisService) ExportAgvcNbqHistory(ctx context.Context, info agvcReq.AgvcNbqHisSearch) (fileBuffer []byte, fileName string, err error) {
    ...
    newList := make([]agvc.AgvcNbq, 0)
    for _, nbqHis := range list {
        localTime := utils.FormatTimeToShanghai(*nbqHis.Ctime)
        nbqHis.Ctime = &localTime
        newList = append(newList, nbqHis)
    }
    ...
    // 写入数据
    for rowIndex, record := range newList {
