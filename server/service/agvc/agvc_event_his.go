...
// ReceiveEvent 接收事件推送并入库
func (agvcEventHisService *AgvcEventHisService) ReceiveEvent(ctx context.Context, event *agvc.Event) (err error) {
    // 将Event转换为AgvcEventHis
    psidStr := intToString(event.Psid)
    eqidStr := intToString(event.Eqid)
    eqTypeStr := intToString(event.EqType)
    dataTypeStr := intToString(event.DataType)
    
    // 将datapoint字符串转为int64
    var datapointInt int64
    if _, err := fmt.Sscanf(event.Datapoint, "%d", &datapointInt); err != nil {
        // 如果转换失败，记录日志但继续处理
        global.GVA_LOG.Warn("datapoint转换失败", zap.String("datapoint", event.Datapoint), zap.Error(err))
        datapointInt = 0
    }
    
    // 将Unix时间戳转换为time.Time
    recordTime := time.Unix(event.HappenTime, 0)
    
    // 默认未读
    isRead := "0"
    
    agvcEventHis := &agvc.AgvcEventHis{
        Psid:       &psidStr,
        Eqid:       &eqidStr,
        EqType:     &eqTypeStr,
        DataType:   &dataTypeStr,
        Datapoint:  &datapointInt,
