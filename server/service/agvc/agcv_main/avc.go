...
func (s *avc) executeRemoteReactiveControl(bwdNo int, requiredDeltaQ float64) error {
	...
	for _, dev := range availableDevices {
		...
		if brand == cons.INVERTER_BRAND_HUAWEI {
			//获取华为逆变器的[无功]调节指令
			pointID := cons.HUAWEI_POWER_ADJUST_VALUE
			...
			if err != nil || currentAdjustMode == 0 {
				global.GVA_LOG.Warn("无法获取当前有功调节指令值，跳过该逆变器",
					zap.Int("inverterNo", *dev.InverterNo),
					zap.Error(err))
				continue
			}
			if currentAdjustMode == cons.HUAWEI_WG_MODE_QS {
				// 计算目标Q/S比值
				// 获取设备当前视在功率
				...
				// 使用品牌接口进行AVC调控
				if err := brandImpl.ControlAVCByReactivePower(&dev, targetQS); err != nil {
					global.GVA_LOG.Error("逆变器AVC控制失败（Q/S模式）",
						zap.Int("inverterNo", *dev.InverterNo),
