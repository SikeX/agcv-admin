...
func (s *agc) executeAGCCycle(bwdNo int) error {
	...
	defer func() {
		if sendErr := s.sendAGCResultToDispatch(bwdNo, config); sendErr != nil {
			global.GVA_LOG.Error("发送AGC结果到调度失败",
				zap.Int("bwdNo", bwdNo),
				zap.Error(sendErr))
		}
		global.GVA_LOG.Info("==============AGC控制周期结束==============")
	}()
	...
	if isRemoteControl {
		// 执行远程调控逻辑
		return s.executeRemoteAGCControl(bwdNo, config)
	}
...
func (s *agc) executeRemoteAGCControl(bwdNo int, config agvc.AgvcBwdSetting) error {
	...
	// 	return nil
	// }
	agcRemoteMode := config.AgcRemoteMode != nil && *config.AgcRemoteMode == 1
	...
	execVal := 0.0
	if config.AgcDispatchExecValue != nil {
		execVal = *config.AgcDispatchExecValue
	}
...
func (s *agc) executeLocalAGCControl(bwdNo int, config agvc.AgvcBwdSetting) error {
	...
	// 步骤2：检查AGC开/闭环状态
	isOpenLoop := config.AgcLoopStatus != nil && *config.AgcLoopStatus == 1
...
func (s *agc) executeRemoteOpenLoopControl(bwdNo int, execVal float64) error {
	...
	for _, inv := range inverters {
		// 根据逆变器品牌选择不同的控制方式
		brandCode := cons.INVERTER_BRAND_HUAWEI // 默认华为
		if inv.InverterBrand != nil {
			brandCode = *inv.InverterBrand
		}
		...
		if err != nil {
			...
			// 使用默认方式
			activePowerPoint, err := InverterBrandMapper.GetActivePowerPoint(brandCode)
			if err != nil {
				global.GVA_LOG.Warn("获取逆变器品牌点位失败，使用默认点位",
					zap.Int("inverterNo", *inv.InverterNo),
					zap.Int("brand", brandCode),
					zap.Error(err))
		...
		if brandCode == cons.INVERTER_BRAND_HUAWEI {
			...
			if mode == 0 {
				...
				if err := brandImpl.ControlAGCByPercentage(&inv, targetPercentage); err != nil {
					global.GVA_LOG.Error("逆变器AGC控制失败（百分比模式）",
						zap.Int("inverterNo", *inv.InverterNo),
						zap.String("brand", brandImpl.GetBrandName()),
						zap.Float64("perInvPower", perInvPower),
				...
				// 使用绝对值模式控制
				if err := brandImpl.ControlAGCByAbsolute(&inv, perInvPower); err != nil {
					global.GVA_LOG.Error("逆变器AGC控制失败（绝对值模式）",
						zap.Int("inverterNo", *inv.InverterNo),
						zap.String("brand", brandImpl.GetBrandName()),
						zap.Float64("perInvPower", perInvPower),
						zap.Error(err))
			...
			// 其他品牌使用默认绝对值模式
			if err := brandImpl.ControlAGCByAbsolute(&inv, perInvPower); err != nil {
				global.GVA_LOG.Error("逆变器AGC控制失败",
					zap.Int("inverterNo", *inv.InverterNo),
					zap.String("brand", brandImpl.GetBrandName()),
					zap.Float64("perInvPower", perInvPower),
					zap.Error(err))
...
func (s *agc) executeRemoteClosedLoopControl(bwdNo int, outputDeviation float64) error {
	...
	// 	// 使用配置中的调度执行值作为上限
	// 	if config.AgcDispatchExecValue != nil {
	// 		activePowerLimit = *config.AgcDispatchExecValue
	// 	}
	// }
	...
	for _, inv := range inverters {
		activePowerLimit += *inv.RatedActivePower
		// 限制调节量不超过逆变器最大调节能力
		...
		if brandCode == cons.INVERTER_BRAND_HUAWEI {
			...
			if mode == 0 {
				...
				if err := brandImpl.ControlAGCByPercentage(&inv, targetPercentage); err != nil {
					global.GVA_LOG.Error("逆变器AGC闭环控制失败（百分比模式）",
						zap.Int("inverterNo", *inv.InverterNo),
						zap.String("brand", brandImpl.GetBrandName()),
				...
				// 使用绝对值模式控制
				if err := brandImpl.ControlAGCByAbsolute(&inv, targetPower); err != nil {
					global.GVA_LOG.Error("逆变器AGC闭环控制失败（绝对值模式）",
						zap.Int("inverterNo", *inv.InverterNo),
						zap.String("brand", brandImpl.GetBrandName()),
						zap.Float64("currentPower", currentPower),
						zap.Float64("targetPower", targetPower),
			...
			// 其他品牌使用默认绝对值模式
			if err := brandImpl.ControlAGCByAbsolute(&inv, targetPower); err != nil {
				global.GVA_LOG.Error("逆变器AGC闭环控制失败",
					zap.Int("inverterNo", *inv.InverterNo),
					zap.String("brand", brandImpl.GetBrandName()),
					zap.Float64("currentPower", currentPower),
					zap.Float64("targetPower", targetPower),
...
func (s *agc) executeClosedLoopControl(bwdNo int, outputDeviation float64) error {
	...
	for _, inv := range inverters {
		...
		if inv.RatedActivePower != nil && *inv.RatedActivePower > 0 {
			...
			if totalRatedPower > 0 {
				// invPowerLimit := activePowerLimit * (*inv.RatedActivePower / totalRatedPower)
				invPowerLimit := *inv.RatedActivePower
				if targetPower > invPowerLimit {
					// 调整实际调节量
...
func (s *agc) executeOpenLoopControl(bwdNo int, execVal float64) error {
	...
	// 发送AGC计算结果到调度
	if err := s.sendAGCResultToDispatch(bwdNo, config); err != nil {
...
func (s *agc) sendAGCResultToDispatch(bwdNo int, config agvc.AgvcBwdSetting) error {
	...
	//根据并网点编号获取逆变器配置
	var inverters []agvc.AgvcNbqSetting
	err = global.GVA_DB.Where("bwd_no = ?", bwdNo).Find(&inverters).Error
	if err != nil {
		global.GVA_LOG.Error("获取逆变器配置失败", zap.Error(err))
		return err
	}
	upPowerLimit := 0.0
	for _, inverter := range inverters {
		upPowerLimit += *inverter.RatedActivePower
	}
	powerLowerLimit := upPowerLimit * 0.05
	results["powerUpperLimit"] = upPowerLimit
