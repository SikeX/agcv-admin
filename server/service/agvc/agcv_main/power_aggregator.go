...
func (pa *powerAggregator) GetBwgPower(bwgNo int, powerType string) (float64, error) {
	...
	switch powerType {
	case "P": // 有功功率
		pointID, err = PointMapper.GetPointID(cons.TYPE_BWG, cons.BWG_YC_ACTIVE_POWER)
	case "Q": // 无功功率
		pointID, err = PointMapper.GetPointID(cons.TYPE_BWG, cons.BWG_YC_REACTIVE_POWER)
	case "S": // 视在功率
		pointID, err = PointMapper.GetPointID(cons.TYPE_BWG, cons.BWG_YC_APPARENT_POWER)
	default:
	...
	power, err := DataStorage.GetDataAsFloat64(1, bwgNo, cons.TYPE_BWG, cons.YC, pointID)
	if err == nil {
...
func (pa *powerAggregator) aggregateFromInverters(bwgNo int, powerType string) (float64, error) {
	...
	switch powerType {
	case "P": // 有功功率
		invPointID, err = PointMapper.GetPointID(cons.TYPE_NBQ, cons.NBQ_YC_ACTIVE_POWER)
		if err != nil {
			invPointID = "10" // 默认值
		}
	case "Q": // 无功功率
		invPointID, err = PointMapper.GetPointID(cons.TYPE_NBQ, cons.NBQ_YC_REACTIVE_POWER)
		if err != nil {
			invPointID = "27" // 默认值
		}
	case "S": // 视在功率
		invPointID, err = PointMapper.GetPointID(cons.TYPE_NBQ, cons.NBQ_YC_APPARENT_POWER)
		if err != nil {
			invPointID = "28" // 默认值
		}
	}
	...
	for _, inv := range inverters {
		invPower, err := DataStorage.GetDataAsFloat64(1, *inv.InverterNo, cons.TYPE_NBQ, cons.YC, invPointID)
		if err != nil {
