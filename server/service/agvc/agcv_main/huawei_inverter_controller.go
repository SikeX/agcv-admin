...
func (h *HuaweiInverterController) ControlAGC(inverter *agvc.AgvcNbqSetting, mode HuaweiControlMode, targetValue float64) error {
    ...
    case HuaweiModePercentage:
        ...
        valueCommands = map[int]interface{}{
            HW_REG_ACTIVE_VALUE: valueInt,
            HW_REG_ACTIVE_PERCENT: valueInt, // 同时设置百分比降额寄存器
        }
    ...
    case HuaweiModeAbsolute:
        ...
        valueCommands = map[int]interface{}{
            HW_REG_ACTIVE_VALUE: valueInt,
            HW_REG_ACTIVE_KW: valueInt, // 同时设置固定值降额寄存器
        }
...
func (h *HuaweiInverterController) ControlAVC(inverter *agvc.AgvcNbqSetting, mode HuaweiReactiveMode, targetValue float64) error {
    ...
    case HuaweiReactiveModePF:
        ...
        valueCommands = map[int]interface{}{
            HW_REG_REACTIVE_VALUE: valueInt,
            HW_REG_REACTIVE_PF: valueInt, // 同时设置功率因数寄存器
        }
    ...
    case HuaweiReactiveModeQS:
        ...
        valueCommands = map[int]interface{}{
            HW_REG_REACTIVE_VALUE: valueInt,
            HW_REG_REACTIVE_QS: valueInt, // 同时设置Q/S寄存器
        }
...
func (h *HuaweiInverterController) ControlAGCByPercentage(inverter *agvc.AgvcNbqSetting, targetPercentage float64) error {
    ...
    valueCommands := map[int]interface{}{
        HW_REG_ACTIVE_PERCENT: valueInt, // 有功功率百分比降额寄存器
    }
...
func (h *HuaweiInverterController) ControlAGCByAbsolute(inverter *agvc.AgvcNbqSetting, targetPowerKW float64) error {
    ...
    valueCommands := map[int]interface{}{
        HW_REG_ACTIVE_KW: valueInt, // 有功功率固定值降额(kW)寄存器
    }
...
func (h *HuaweiInverterController) ControlAVCByPowerFactor(inverter *agvc.AgvcNbqSetting, targetPF float64) error {
    ...
    // 1. 设置无功调节模式为功率因数模式
    modeCommands := map[int]interface{}{
        HW_REG_REACTIVE_MODE: int(HuaweiReactiveModePF), // 功率因数模式
    }
    ...
    valueCommands := map[int]interface{}{
        HW_REG_REACTIVE_PF: valueInt, // 无功功率补偿(功率因数)寄存器
    }
    ...
    // 3. 发送调节指令(执行)
    execCommands := map[int]interface{}{
        HW_REG_REACTIVE_CMD: 1, // 无功调节指令执行
    }
...
func (h *HuaweiInverterController) ControlAVCByReactivePower(inverter *agvc.AgvcNbqSetting, targetQS float64) error {
    ...
    // 1. 设置无功调节模式为Q/S模式
    modeCommands := map[int]interface{}{
        HW_REG_REACTIVE_MODE: int(HuaweiReactiveModeQS), // Q/S模式
    }
    ...
    valueCommands := map[int]interface{}{
        HW_REG_REACTIVE_QS: valueInt, // 无功功率补偿(Q/S)寄存器
    }
...
func (h *HuaweiInverterController) SetControlGradient(inverter *agvc.AgvcNbqSetting, activePowerGradient, reactivePowerGradient float64) error {
    ...
    if activePowerGradient > 0 {
        commands[HW_REG_ACTIVE_GRADIENT] = int(activePowerGradient * 1000)
    }

    // 设置无功功率变化梯度(0.001精度)
    if reactivePowerGradient > 0 {
        commands[HW_REG_REACTIVE_GRADIENT] = int(reactivePowerGradient * 1000)
    }
...
// GetPointRegister 获取点位对应的寄存器地址
func (h *HuaweiInverterController) GetPointRegister(pointID string) (int, error) {
    switch pointID {
    case HW_POINT_REACTIVE_GRADIENT:
        return HW_REG_REACTIVE_GRADIENT, nil
    case HW_POINT_ACTIVE_GRADIENT:
        return HW_REG_ACTIVE_GRADIENT, nil
    case HW_POINT_MAINTAIN_TIME:
        return HW_REG_MAINTAIN_TIME, nil
    case HW_POINT_ACTIVE_KW:
        return HW_REG_ACTIVE_KW, nil
    case HW_POINT_REACTIVE_PF:
        return HW_REG_REACTIVE_PF, nil
    case HW_POINT_REACTIVE_QS:
        return HW_REG_REACTIVE_QS, nil
    case HW_POINT_ACTIVE_PERCENT:
        return HW_REG_ACTIVE_PERCENT, nil
    case HW_POINT_ACTIVE_W:
        return HW_REG_ACTIVE_W, nil
    case HW_POINT_ACTIVE_MODE:
        return HW_REG_ACTIVE_MODE, nil
    case HW_POINT_ACTIVE_VALUE:
        return HW_REG_ACTIVE_VALUE, nil
    case HW_POINT_ACTIVE_CMD:
        return HW_REG_ACTIVE_CMD, nil
    case HW_POINT_REACTIVE_MODE:
        return HW_REG_REACTIVE_MODE, nil
    case HW_POINT_REACTIVE_VALUE:
        return HW_REG_REACTIVE_VALUE, nil
    case HW_POINT_REACTIVE_CMD:
        return HW_REG_REACTIVE_CMD, nil
    default:
        return 0, fmt.Errorf("未找到点位定义: %s", pointID)
    }
}
...
func (h *HuaweiInverterController) ConvertToRegisterValue(pointID string, realValue float64) (int, error) {
    // 先验证值范围
    if err := h.ValidateControlValue(pointID, realValue); err != nil {
        return 0, err
    }

    // 根据点位类型进行转换
    switch pointID {
    case HW_POINT_ACTIVE_PERCENT: // 有功功率百分比降额(0.1%精度)
        return int(realValue * 10), nil
    case HW_POINT_ACTIVE_KW: // 有功功率固定值降额(kW, 0.1精度)
        return int(realValue * 10), nil
    case HW_POINT_REACTIVE_PF: // 功率因数(0.001精度)
        return int(realValue * 1000), nil
    case HW_POINT_REACTIVE_QS: // Q/S(0.001精度)
        return int(realValue * 1000), nil
    case HW_POINT_REACTIVE_GRADIENT, HW_POINT_ACTIVE_GRADIENT: // 变化梯度(0.001精度)
        return int(realValue * 1000), nil
    case HW_POINT_MAINTAIN_TIME: // 维持时间(秒)
        return int(realValue), nil
    case HW_POINT_ACTIVE_MODE: // 有功调节模式
        return int(realValue), nil
    case HW_POINT_REACTIVE_MODE: // 无功调节模式
        return int(realValue), nil
    case HW_POINT_ACTIVE_CMD, HW_POINT_REACTIVE_CMD: // 调节指令
        return int(realValue), nil
    case HW_POINT_ACTIVE_VALUE, HW_POINT_REACTIVE_VALUE: // 调节值
        return int(realValue), nil
    default:
        return 0, fmt.Errorf("不支持的点位ID: %s", pointID)
    }
}
...
func (h *HuaweiInverterController) ConvertFromRegisterValue(pointID string, registerValue int) (float64, error) {
    ...
    switch pointID {
    case HW_POINT_ACTIVE_PERCENT: // 有功功率百分比降额(0.1%精度)
        return float64(registerValue) / 10, nil
    case HW_POINT_ACTIVE_KW: // 有功功率固定值降额(kW, 0.1精度)
        return float64(registerValue) / 10, nil
    case HW_POINT_REACTIVE_PF: // 功率因数(0.001精度)
        return float64(registerValue) / 1000, nil
    case HW_POINT_REACTIVE_QS: // Q/S(0.001精度)
        return float64(registerValue) / 1000, nil
    case HW_POINT_REACTIVE_GRADIENT, HW_POINT_ACTIVE_GRADIENT: // 变化梯度(0.001精度)
        return float64(registerValue) / 1000, nil
    case HW_POINT_MAINTAIN_TIME: // 维持时间(秒)
        return float64(registerValue), nil
    case HW_POINT_ACTIVE_MODE: // 有功调节模式
        return float64(registerValue), nil
    case HW_POINT_REACTIVE_MODE: // 无功调节模式
        return float64(registerValue), nil
