...
func (agvcBwdHisService *AgvcBwdHisService) GetAgvcBwdHistory(ctx context.Context, eqid, startTime, endTime string) ([]map[string]interface{}, error) {
    ...
    `, global.GVA_CONFIG.InfluxDB.Bucket, startTime, endTime, global.GVA_CONFIG.InfluxDB.GetMeasurement(), eqid)
    ...
    if len(historyData) == 0 {
        global.GVA_LOG.Info(fmt.Sprintf("并网点%s在时间范围%s到%s内没有历史数据", eqid, startTime, endTime))
...
func (agvcBwdHisService *AgvcBwdHisService) GetBwdRealtimeData(ctx context.Context, number string) (map[string][]map[string]interface{}, error) {
    ...
    `, global.GVA_CONFIG.InfluxDB.Bucket, startTime.Format(time.RFC3339), endTime.Format(time.RFC3339), global.GVA_CONFIG.InfluxDB.GetMeasurement(), number)
