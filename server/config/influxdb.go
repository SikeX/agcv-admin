...
// GetAgvcRetention 获取agvc储存桶保留时间，如果未配置则返回默认值 24h
func (i InfluxDB) GetAgvcRetention() string {
    if i.AgvcRetention == "" {
        return "24h"
    }
    return i.AgvcRetention
}

// GetNbqRetention 获取逆变器储存桶保留时间，如果未配置则返回默认值 168h
func (i InfluxDB) GetNbqRetention() string {
    if i.NbqRetention == "" {
        return "168h"
    }
    return i.NbqRetention
}
